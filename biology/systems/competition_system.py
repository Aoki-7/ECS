...
class CompetitionSystem(System):
    ...
    def update(self, world: World, dt: float = 1.0):
        ...
        for c in competitors:
            c["comp"].light_competition_score = 0.0
            c["comp"].water_competition_score = 0.0
    ...
    def _collect_competitors(self, world: World) -> List[Dict]:
        """收集所有带竞争组件的实体"""
        competitors = []
        for entity, (comp, morph, pheno, space) in world.get_components(
    ...
    def _compete_pair(self, c1: Dict, c2: Dict):
        ...
        # --- 光照竞争（冠层遮阴）---
        canopy1 = c1["morph"].height * c1["comp"].canopy_radius * 0.1
        canopy2 = c2["morph"].height * c2["comp"].canopy_radius * 0.1
        ...
        if distance < max_canopy:
            ...
            if h1 > h2:
                # c1 赢，c2 受遮阴
                c1["comp"].light_competition_score += 0.1 * c1[
                    "comp"
                ].competitive_ability
                c2["comp"].light_competition_score += 0.3 * (
                    h2 / max(h1, 1.0)
                )
            elif h2 > h1:
                c2["comp"].light_competition_score += 0.1 * c2[
                    "comp"
                ].competitive_ability
                c1["comp"].light_competition_score += 0.3 * (
                    h1 / max(h2, 1.0)
                )
            else:
                # 高度相同，互相抑制
                c1["comp"].light_competition_score += 0.2
                c2["comp"].light_competition_score += 0.2

        # --- 水分竞争（根系重叠）---
        root1 = c1["comp"].root_radius
        root2 = c2["comp"].root_radius
        max_root = max(root1, root2)

        if distance < max_root:
            if root1 > root2:
                c1["comp"].water_competition_score += 0.1
                c2["comp"].water_competition_score += 0.2
            elif root2 > root1:
                c2["comp"].water_competition_score += 0.1
                c1["comp"].water_competition_score += 0.2
            else:
                c1["comp"].water_competition_score += 0.15
                c2["comp"].water_competition_score += 0.15
    ...
    def _apply_competition_effects(self, competitors: List[Dict]):
        ...
        for c in competitors:
            ...
            # 光照竞争失败 → 降低光合速率
            if comp.light_competition_score > 0:
                shade_penalty = min(0.7, comp.light_competition_score * 0.3)
            ...
            # 水分竞争失败 → 降低水分利用效率
            if comp.water_competition_score > 0:
                current_wue = pheno.get("water_use_efficiency", 0.05)
                pheno.set_trait(
                    Trait(
                        name="water_use_efficiency",
                        value=current_wue
                        * (1.0 - min(0.5, comp.water_competition_score * 0.2)),
                        source="competition",
                    )
                )
