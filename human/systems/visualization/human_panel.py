...
class HumanStatePanel:
    ...
    def render(self, world: "World", step_count: int = 0) -> Panel:
        ...
        table.add_column("饥饿", width=14)
        table.add_column("口渴", width=14)
        table.add_column("精力", width=14)
        table.add_column("情绪", width=8, justify="center")
        table.add_column("意图", width=10, no_wrap=True)
        table.add_column("动作", width=10, no_wrap=True)
        ...
        for entity, _ in world.get_components(HumanComponent):
            ...
            hunger_bar = self._bar(needs.hunger if needs else 0, 100, 8)
            thirst_bar = self._bar(needs.thirst if needs else 0, 100, 8)
            energy_bar = self._energy_bar(needs.energy if needs else 0, 100, 8)
